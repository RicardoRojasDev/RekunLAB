# SQL COMPLETO - Migración y Carga de Catálogo Real

**Archivo:** `supabase/migrations/20260419_modulo_15_catalogo_real.sql`
...
## PARTE 2: NOTAS DE IMPLEMENTACIÓN

### Requisitos Previos

1. **Tabla `producto` debe existir** con las columnas base (from Módulo 14)
2. **Tablas de referencia deben existir:**
   - `marca_producto`
   - `nivel_comercial_producto`
   - `categoria_producto`
   - `estado_entidad` (con estado 'activo' para producto)
   - `material` (para PLA)
3. **Índices se crearán automáticamente** como parte del script

### Datos Insertados

- **35 Productos:** 18 Filamentos, 12 Impresoras, 5 Packs
- **4 Marcas:** Rekün LAB, Creality, Bambu Lab, Anycubic
- **9 Niveles Comerciales:** Base, Premium, Económica, Intermedia, Avanzada, Industrial, Maker, Wow, Producción
- **3 Categorías:** Filamentos PLA, Impresoras 3D, Packs

### Campos en METADATOS (JSONB)

Cada producto guarda en `metadatos`:
```json
{
  "nombre_completo": "String - Nombre completo descriptivo",
  "acabado": "String - Mate, Satinado, Brillante, Silk, Semi Mate, Variable",
  "efecto": "String - Ninguno, UV, Partículas, Cambio Angular, Temperatura, etc.",
  "color_hex": "String - Código hexadecimal del color (#XXXXXX)",
  "compatible_pla": "Boolean - Compatibilidad con PLA"
}
```
...
SELECT tipo_producto, COUNT(*) FROM public.producto GROUP BY tipo_producto;
EOF

# 3. Validar índices
SELECT * FROM pg_indexes WHERE tablename = 'producto' AND indexname LIKE 'producto_%';
```
